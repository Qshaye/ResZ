...
class DMixer(nn.Module):
    ...
    def forward(self, agent_qs, states, target):  # agent_qs(batch_size, episode_length, n_agents, n_quantiles)
        ...
        q_mixture = agent_qs.sum(dim=2, keepdim=True)     # q_mixture: 把多个智能体的 Z 加起来 (Zsum)
        assert q_mixture.shape == (batch_size, episode_length, 1, n_rnd_quantiles)
        q_vals_expected = agent_qs.mean(dim=3, keepdim=True)  # q_vals_expected: 计算每个agent的 Q
        q_vals_sum = q_vals_expected.sum(dim=2, keepdim=True)   # q_vals_sum: agents的 Q 加起来
        ...
        # 把每个智能体的 Q 给QMIX计算Z_mean
        q_joint_expected = self.forward_qmix(q_vals_expected, states)
        assert q_joint_expected.shape == (batch_size, episode_length, 1, 1)

        # Shape network
        q_vals_sum = q_vals_sum.expand(-1, -1, -1, n_rnd_quantiles)  # 为了与分布Z相减
        q_joint_expected = q_joint_expected.expand(-1, -1, -1, n_rnd_quantiles)  # 同上
        ...
        q_joint = q_mixture - q_vals_sum + q_joint_expected  # ! 分解公式：Zmean-Zshape: Zmean-(Zsum-Qsum)
    ...
    def forward_qmix(self, agent_qs, states):
        ...
        assert states.shape == (batch_size * episode_length, self.state_dim)
        # First layer
        w1 = th.abs(self.hyper_w_1(states))  # 保证权重大于=0
