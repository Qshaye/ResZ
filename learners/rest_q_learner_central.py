...
def get_ws(resq_version, condition, td_error):
    if resq_version == "v3":
        ws = th.where(condition, th.zeros_like(td_error), th.ones_like(td_error))  # 是最优 w=0 不是最优 w=1
    return ws
...
class RestQLearnerCentral:
    ...
    def __init__(self, mac, scheme, logger, args):
        ...
        self.rest_mixer = QMixerCentralFF(args)  # residual func 没有单调性限制的mixer
        ...
        self.target_rest_mixer = copy.deepcopy(self.rest_mixer)  #added for RESTQ
        print('Mixer Size: ')
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # weighted qmix: y_i = r + \gamma Q^*(s', \tau, argmax_u'Q_tot)
        ...
        if self.args.hysteretic_qmix:  # OW-QMIX
            ...
            is_max_action = (actions == cur_max_actions[:, :-1]).min(dim=2)[0]  # 判断多个 agent 选择的 action 组合是否 argmax Qtot的，只要有一个是False就是False
            ...
            # 是最优动作时 w_r=0 不是则为1 把Qjt变小 resq(5)  （Qr是<0的）
