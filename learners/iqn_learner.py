...
class IQNLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out = th.stack(mac_out, dim=1)  # Concat over time: add a dim (episode_length)
        ...
        rnd_quantiles = rnd_quantiles[:, :-1]  # ? 为什么episode_length 比max_seq_length小
        assert rnd_quantiles.shape == (batch.batch_size, episode_length, n_quantile_groups, self.n_quantiles)

        # Pick the Q-Values for the actions taken by each agent Z(s, a)
        actions_for_quantiles = actions.unsqueeze(4).expand(-1, -1, -1, -1, self.n_quantiles)
        del actions
        chosen_action_qvals = th.gather(mac_out[:,:-1], dim=3, index=actions_for_quantiles).squeeze(3)  # Remove the last dim
        ...
        if self.args.double_q:
            ...
            cur_max_actions = mac_out_detach[:,1:].mean(dim=4).max(dim=3, keepdim=True)[1] # 选择target分布的index
            del mac_out_detach
            ...
            cur_max_actions = target_mac_out.mean(dim=4).max(dim=3, keepdim=True)[1]  # calculate Q -> argmax Q
            assert cur_max_actions.shape == (batch.batch_size, episode_length, self.args.n_agents, 1)
            cur_max_actions = cur_max_actions.unsqueeze(4).expand(-1, -1, -1, -1, self.n_target_quantiles)
            target_max_qvals = th.gather(target_mac_out, 3, cur_max_actions).squeeze(3)
            del cur_max_actions
        ...
        # Mix
        if self.mixer is not None:
            target_max_qvals = self.target_mixer(target_max_qvals, batch["state"][:, 1:], target=True) # Zjt
            chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1], target=False) # Ztot
        ...
        # u is the signed distance matrix    Z - \theta
        ...
        # The abs term in quantile huber loss 实现公式里的 1-tau/tau
        ...
        # Huber loss
        loss = F.smooth_l1_loss(u, th.zeros(u.shape).cuda(), reduction='none') # huber loss
        del u
        ...
        assert mask.shape == (batch.batch_size, episode_length, 1)
        mask = mask.expand_as(loss)    # mask来自于filled属性  把无效经验产生的loss去掉
